...
annotated program. The invariants themselves do not involve memory.
  ...
  bincaml: [INFO] Submitting 11 predicates and 13 clauses to solver
  bincaml: [INFO] Solver returned sat; extracted 11 definitions
...
addressed [store le]/[load le] instructions are preserved:
  ...
  $ grep -E '(store|load) le ' chc_mem_out.il
       $mem:(bv64->bv8) := store le $mem:(bv64->bv8) 0x100:bv64 i_3:bv64 64;
