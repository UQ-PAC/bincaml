...
CHC spec inlining (Step 7) — [helper] declares [requires]/[ensures], so the
pass verifies its body against the spec independently and uses the spec
(not the helper's [enter]/[exit] predicates) at the call site in [main].
...
Spec inlining means main's assertion was verified using helper's [ensures]
