...
let conditional_writes_wrapped_intervals_guarantee_conditions_1 () =
  let test = "Basic mutual-exclusion detection using the conditional-writes domain with wrapped intervals." in
  let prog_str =
{|
prog entry @dummy;
...
|}
  ...
  in
  let x = Program.global_vars program |> Iter.find_pred_exn (fun v -> v |> Var.name |> String.equal "$x") in
  ...
  let x_eq_2 = StateAbstraction.update x (WrappedIntervalsLattice.interval two two) StateAbstraction.top in
  ...
  List.iter (fun (proc, guar) ->
    ...
    if String.equal proc_name "@t1" then
      ...
      Alcotest.(check bool) test true (StateAbstraction.equal x_eq_1 write_cond)
    else if String.equal proc_name "@t2" then
      let write_cond = I.VarSetMap.find x_set guar in
      Alcotest.(check bool) test true (StateAbstraction.equal x_eq_2 write_cond)
    else
      Alcotest.(check bool) test true (String.equal proc_name "@dummy")
  )

let conditional_writes_wrapped_intervals_guarantee_conditions_2 () =
  let test = "Attempt to fail to infer mutual-exclusion using the conditional-writes domain with wrapped intervals." in
  let prog_str =
{|
prog entry @dummy;
...
|}
  ...
  List.iter (fun (proc, guar) ->
    ...
    if String.equal proc_name "@t1" then
      let write_cond = I.VarSetMap.find x_set guar in
      Alcotest.(check bool) test true (StateAbstraction.equal StateAbstraction.top write_cond)
    else if String.equal proc_name "@t2" then
      let write_cond = I.VarSetMap.find x_set guar in
      Alcotest.(check bool) test true (StateAbstraction.equal StateAbstraction.top write_cond)
    else
...
let tests = [
  ...
  ("conditional_writes_wrapped_intervals_guarantee_conditions_1", conditional_writes_wrapped_intervals_guarantee_conditions_1);
  ("conditional_writes_wrapped_intervals_guarantee_conditions_2", conditional_writes_wrapped_intervals_guarantee_conditions_2)
]
