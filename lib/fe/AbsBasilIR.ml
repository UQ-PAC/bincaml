...
and moduleT = Module1 of decl list
and semicolons = Semicolons_Empty | Semicolons_Some of semicolons
...
and decl =
  ...
  | Decl_ProgWithSpec of
      procIdent * attribSet * beginList * progSpec list * endList
  | Decl_Proc of
      procIdent * params list * params list * attribSet * funSpec list * procDef

and procDef = ProcDef_Empty | ProcDef_Some of beginList * block list * endList
and intType = IntType1 of iNTTYPE
and boolType = BoolType1 of bOOLTYPE
and mapType = MapType1 of typeT * typeT
and bVType = BVType1 of bVTYPE
...
and intVal = IntVal_Hex of integerHex | IntVal_Dec of integerDec
and bVVal = BVVal1 of intVal * bVType
and endian = Endian_Little | Endian_Big
and assignment = Assignment1 of lVar * expr
...
and localVar = LocalVar1 of localIdent * typeT
and globalVar = GlobalVar1 of globalIdent * typeT
and var = VarLocalVar of localVar | VarGlobalVar of globalVar
and namedCallReturn = NamedCallReturn1 of lVar * localIdent
...
and namedCallArg = NamedCallArg1 of localIdent * expr
...
and lVar = LVar_Local of localVar | LVar_Global of globalVar
and stmtWithAttrib = StmtWithAttrib1 of stmt * attribSet
and jumpWithAttrib = JumpWithAttrib1 of jump * attribSet
and phiExpr = PhiExpr1 of blockIdent * var
and phiAssign = PhiAssign1 of lVar * phiExpr list
...
and attrKeyValue = AttrKeyValue1 of bIdent * attr
...
and params = Params1 of localIdent * typeT
and value = Value_BV of bVVal | Value_Int of intVal | Value_True | Value_False
...
and lambdaDef = LambdaDef1 of localVar list * lambdaSep * expr
...
and unOp = UnOpBVUnOp of bVUnOp | UnOp_boolnot | UnOp_intneg | UnOp_booltobv1
and eqOp = EqOp_eq | EqOp_neq
and bVUnOp = BVUnOp_bvnot | BVUnOp_bvneg
...
and boolBinOp = BoolBinOp_booland | BoolBinOp_boolor | BoolBinOp_boolimplies
and requireTok = RequireTok_require | RequireTok_requires
and ensureTok = EnsureTok_ensure | EnsureTok_ensures
...
and progSpec = ProgSpec_Rely of expr | ProgSpec_Guarantee of expr
