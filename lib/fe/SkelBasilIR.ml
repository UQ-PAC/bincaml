...
module SkelBasilIR = struct
  ...
  let rec transBVTYPE (x : bVTYPE) : result =
    match x with BVTYPE string -> failure x

  and transINTTYPE (x : iNTTYPE) : result =
    match x with INTTYPE string -> failure x

  and transBOOLTYPE (x : bOOLTYPE) : result =
    match x with BOOLTYPE string -> failure x

  and transBIdent (x : bIdent) : result =
    match x with BIdent string -> failure x

  and transLocalIdent (x : localIdent) : result =
    match x with LocalIdent string -> failure x

  and transGlobalIdent (x : globalIdent) : result =
    match x with GlobalIdent string -> failure x

  and transBlockIdent (x : blockIdent) : result =
    match x with BlockIdent string -> failure x

  and transProcIdent (x : procIdent) : result =
    match x with ProcIdent string -> failure x

  and transBeginList (x : beginList) : result =
    match x with BeginList string -> failure x

  and transEndList (x : endList) : result =
    match x with EndList string -> failure x

  and transBeginRec (x : beginRec) : result =
    match x with BeginRec string -> failure x

  and transEndRec (x : endRec) : result =
    match x with EndRec string -> failure x

  and transLambdaSep (x : lambdaSep) : result =
    match x with LambdaSep string -> failure x

  and transStr (x : str) : result = match x with Str string -> failure x

  and transIntegerHex (x : integerHex) : result =
    match x with IntegerHex string -> failure x

  and transIntegerDec (x : integerDec) : result =
    match x with IntegerDec string -> failure x

  and transModule (x : moduleT) : result =
    match x with Module1 decls -> failure x

  and transSemicolons (x : semicolons) : result =
    match x with
  ...
  and transDecl (x : decl) : result =
    ...
    | Decl_ProgWithSpec (procident, attribset, beginlist, progspecs, endlist) ->
        failure x
    | Decl_Proc (procident, paramss0, paramss, attribset, funspecs, procdef) ->
        failure x

  and transProcDef (x : procDef) : result =
    match x with
    | ProcDef_Empty -> failure x
    | ProcDef_Some (beginlist, blocks, endlist) -> failure x

  and transIntType (x : intType) : result =
    match x with IntType1 inttype -> failure x

  and transBoolType (x : boolType) : result =
    match x with BoolType1 booltype -> failure x

  and transMapType (x : mapType) : result =
    match x with MapType1 (type'0, type') -> failure x

  and transBVType (x : bVType) : result =
    match x with BVType1 bvtype -> failure x
  ...
  and transIntVal (x : intVal) : result =
    match x with
    | IntVal_Hex integerhex -> failure x
    | IntVal_Dec integerdec -> failure x

  and transBVVal (x : bVVal) : result =
    match x with BVVal1 (intval, bvtype) -> failure x

  and transEndian (x : endian) : result =
    match x with Endian_Little -> failure x | Endian_Big -> failure x

  and transAssignment (x : assignment) : result =
    match x with Assignment1 (lvar, expr) -> failure x
  ...
  and transLocalVar (x : localVar) : result =
    match x with LocalVar1 (localident, type') -> failure x

  and transGlobalVar (x : globalVar) : result =
    match x with GlobalVar1 (globalident, type') -> failure x

  and transVar (x : var) : result =
    match x with
    | VarLocalVar localvar -> failure x
    | VarGlobalVar globalvar -> failure x

  and transNamedCallReturn (x : namedCallReturn) : result =
    match x with NamedCallReturn1 (lvar, localident) -> failure x

  and transLVars (x : lVars) : result =
    match x with
  ...
  and transNamedCallArg (x : namedCallArg) : result =
    match x with NamedCallArg1 (localident, expr) -> failure x

  and transCallParams (x : callParams) : result =
    match x with
    | CallParams_Exprs exprs -> failure x
    | CallParams_Named namedcallargs -> failure x

  and transJump (x : jump) : result =
    match x with
  ...
  and transLVar (x : lVar) : result =
    match x with
    | LVar_Local localvar -> failure x
    | LVar_Global globalvar -> failure x

  and transStmtWithAttrib (x : stmtWithAttrib) : result =
    match x with StmtWithAttrib1 (stmt, attribset) -> failure x

  and transJumpWithAttrib (x : jumpWithAttrib) : result =
    match x with JumpWithAttrib1 (jump, attribset) -> failure x

  and transPhiExpr (x : phiExpr) : result =
    match x with PhiExpr1 (blockident, var) -> failure x

  and transPhiAssign (x : phiAssign) : result =
    match x with PhiAssign1 (lvar, phiexprs) -> failure x

  and transBlock (x : block) : result =
    match x with
    | Block_NoPhi
        ( blockident,
          attribset,
  ...
  and transAttrKeyValue (x : attrKeyValue) : result =
    match x with AttrKeyValue1 (bident, attr) -> failure x

  and transAttribSet (x : attribSet) : result =
    match x with
    | AttribSet_Some (beginrec, attrkeyvalues, semicolons, endrec) -> failure x
    | AttribSet_Empty -> failure x

  and transAttr (x : attr) : result =
    match x with
  ...
  and transParams (x : params) : result =
    match x with Params1 (localident, type') -> failure x

  and transValue (x : value) : result =
    match x with
  ...
  and transLambdaDef (x : lambdaDef) : result =
    match x with LambdaDef1 (localvars, lambdasep, expr) -> failure x

  and transBinOp (x : binOp) : result =
    match x with
  ...
  and transEqOp (x : eqOp) : result =
    match x with EqOp_eq -> failure x | EqOp_neq -> failure x

  and transBVUnOp (x : bVUnOp) : result =
    match x with BVUnOp_bvnot -> failure x | BVUnOp_bvneg -> failure x

  and transBVBinOp (x : bVBinOp) : result =
    match x with
  ...
  and transBVLogicalBinOp (x : bVLogicalBinOp) : result =
    match x with
  ...
  and transIntBinOp (x : intBinOp) : result =
    match x with
  ...
  and transIntLogicalBinOp (x : intLogicalBinOp) : result =
    match x with
  ...
  and transProgSpec (x : progSpec) : result =
    match x with
