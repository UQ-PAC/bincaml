...
let is_atom (f : sexp) = match f with `Atom _ -> true | `List _ -> false
let to_list (f : sexp) = match f with `Atom _ -> None | `List xs -> Some xs
...
(** Symbols are either simple or quoted (c.f. SMTLIB v2.6 S3.1). This predicate
    indicates whether a character is allowed in a simple symbol. Note that only
...
(** Make a bit-vector literal. Uses hex representation if the size is a multiple
    of 4, and binary otherwise. *)
...
(** [bv_extract i j x] is a sub-vector of [x]. [i] is the larger bit index, [j]
...
(** [arr_const kt vt v] is an array of type [Array kt vt] where all elements are
    [v]. [v] should be of type [vt]. *)
...
(** Some solvers support theories outside of SMTLIB and, unfortunately, there is
    no standard for what things are called. We use this flag to decide how to
    generate terms for those cases. *)
...
(** [declare_sort name arity] declares a type [name], which expect [arity] type
    parameters. *)
...
(** Get all definitions currently in scope. Only valid after a [Sat] result. See
    also {!model_eval}. *)
let get_model s =
  let open Sexp in
...
(** Expand let-definitions in this term. NOTE: this is intended to be used
    mostly on models generated by the solver (e.g., `get-value` in Z3 sometimes
    contains `let`). As such we assume that `forall` and `exist` won't occur,
    and so we don't need to check for variable capture. *)
