...
(** SimpleSMT Interface adapted from:

    {:https://galoisinc.github.io/simple-smt-ocaml/simple_smt/Simple_smt/index.html}

    {:https://github.com/NatKarmios/simple-smt-ocaml/tree/main} *)
...
module Config = struct
  ...
  (** Some solvers support theories outside of SMTLIB and, unfortunately, there
      is no standard for what things are called. We use this flag to decide how
      to generate terms for those cases. *)
  ...
  let quiet_log =
    { send = (fun _ -> ()); receive = (fun _ -> ()); stop = (fun _ -> ()) }

  let printf_log =
    {
      send = (fun s -> Printf.printf "[->] %s\n%!" s);
      receive = (fun s -> Printf.printf "[<-] %s\n%!" s);
      stop = (fun _ -> ());
    }
...
module Expr = struct
  (** Functions for building SMT2 S-expressions *)
  ...
  (** Symbols are either simple or quoted (c.f. SMTLIB v2.6 S3.1). This
      predicate indicates whether a character is allowed in a simple symbol.
      Note that only ASCII letters are allowed. *)
  let allowed_simple_char c =
    let co = Char.code c in
    let in_range a b = Char.code a <= co && co <= Char.code b in
    in_range 'a' 'z' || in_range 'A' 'Z' || in_range '0' '9'
    || String.contains "~!@$%^&*_-+=<>.?/" c

  let is_simple_symbol s =
    (not (String.equal "" s)) && String.for_all allowed_simple_char s

  let quote_symbol s = if is_simple_symbol s then s else "|" ^ s ^ "|"

  (** Make an SMT name, quoting if needed. Note that even with quoting the name
      should not contain pipe (|) or backslash (\) *)
  ...
  *)
  ...
  let bv_nat_hex w v =
    atom ("#x" ^ Z.format ("0" ^ string_of_int (w / 4) ^ "x") v)

  (** Bit vector arithmetic negation. *)
  let bv_neg x = app_ "bvneg" [ x ]

  (** A bit-vector represented in binary. The number should fit in the given
      number of bits. *)
  let bv_bin w v =
  ...
  (** Make a bit-vector literal. Uses hex representation if the size is a
      multiple of 4, and binary otherwise. *)
  ...
  (** [bv_extract i j x] is a sub-vector of [x]. [i] is the larger bit index,
      [j] is the smaller one, and indexing is inclusive. *)
  ...
  (** [arr_store arr ix val] is an array that is the same as [arr], except at
      index [ix] it has [val] *)
  ...
  *)
  ...
  (** [set_intersection ext xs ys] has the elements that are in both [xs] and
      [ys] *)
  ...
  *)
  ...
  let is_let (exp : sexp) : (sexp StringMap.t * sexp) option =
    match exp with
    | `List [ `Atom "let"; `List binds; body ] ->
        let add_bind mp bind =
          match bind with
          | `List [ `Atom x; y ] -> StringMap.add x y mp
          | _ -> raise (UnexpectedSolverResponse bind)
        in
        let su = List.fold_left add_bind StringMap.empty binds in
        Some (su, body)
    | _ -> None

  (** Expand let-definitions in this term. NOTE: this is intended to be used
      mostly on models generated by the solver (e.g., `get-value` in Z3
      sometimes contains `let`). As such we assume that `forall` and `exist`
      won't occur, and so we don't need to check for variable capture. *)
  let no_let (exp0 : sexp) : sexp =
    let rec expand su exp =
      match is_let exp with
      | Some (binds, body) ->
          let binds1 = StringMap.map (expand su) binds in
          let jn _ x _ = Some x in
          let su1 = StringMap.union jn binds1 su in
          expand su1 body
      | None -> (
          match exp with
          | `List xs -> `List (List.map (expand su) xs)
          | `Atom x -> (
              match StringMap.find_opt x su with Some e1 -> e1 | None -> exp))
    in
    expand StringMap.empty exp0

  (** Try to decode an s-expression as a boolean Throws
  ...
  (** Try to decode an s-expression as a bitvector of the given width. The 2nd
      argument indicates if the number is signed. Throws
      {!UnexpectedSolverResponse}. *)
  let to_bits w signed (exp : sexp) =
    ...
    | `Atom s ->
        ...
        if tot_len < 2 || not (Char.equal (String.get s 0) '#') then bad ()
        else ();
  ...
  (** Try to decode an s-expression as an integer. Throws
      {!UnexpectedSolverResponse}. *)
  let to_z (exp : sexp) =
    let parse do_neg s =
      try
    ...
    in
    match exp with
    | `Atom s -> parse false s
    | `List [ `Atom "-"; `Atom s ] -> parse true s
    | _ -> raise (UnexpectedSolverResponse exp)

  (** Try to decode an s-expression as a rational number. Throws
      {!UnexpectedSolverResponse}. *)
  let to_q (exp : sexp) =
    let bad () = raise (UnexpectedSolverResponse exp) in
    let rec eval e =
      match e with
      | `Atom s -> Q.of_string s
      | `List [ `Atom "-"; e1 ] -> Q.neg (eval e1)
      | `List [ `Atom "/"; e1; e2 ] -> Q.div (eval e1) (eval e2)
      | _ -> bad ()
    in
    try eval exp with Invalid_argument _ -> bad ()

  (** Try to decode an s-expression as constructor with field values. Throws
      {!UnexpectedSolverResponse}. *)
  ...
  (** {1 Commands}
      The S-Expressions in this section define commands to the SMT solver. These
      can be sent to solver using {!ack_command}. *)
  ...
  (** [declare_sort name arity] declares a type [name], which expect [arity]
      type parameters. *)
  let declare_sort f arity =
    app_ "declare-sort" [ atom f; atom (string_of_int arity) ]

  (** [declare_fun name param_types result_type] declareas a function expecting
      arguments with types [param_types] and computes a value of type
      [result_type]. *)
  ...
  (** [define_fun name params result_type definition] defines a function called
      [name], with the given parameters [(name, type)], that computes a value of
      [result_type], using [definition]. *)
  let define_fun f ps r d =
    let mk_param (x, a) = list [ atom x; a ] in
    app_ "define-fun" [ atom f; list (List.map mk_param ps); r; d ]

  (** [define name type definition] defines a constant [name], of type [type]
  ...
  (** [declare_datatype name type_params cons] define an ADT called [name] with
      type parameters [type_params] and constructors [cons]. Each constructor is
      of the form [(name, fields)]. *)
  ...
  (** [declare_datatypes tys] defines a group of mutually recursive ADTs. Each
      element if `tys` is (name,type params,cons). *)
  let declare_datatypes tys =
    ...
    in
    app_ "declare-datatypes"
      [ list (List.map arity tys); list (List.map def tys) ]
...
module Solver : sig
  ...
  val get_model : t -> sexp
  (** return Sexpr of model following a SAT result *)
  ...
  (** Send an assert to the solver *)

  val eval_expr : t -> sexp -> sexp
  (** Evaluate expr after a SAT result *)

  val eval_exprs : t -> sexp list -> sexp list
  (** Evaluate expr list after a SAT result *)
  ...
  val stop : t -> unit
  (** Tell the solver to stop *)

  val force_stop : t -> unit
  (** Send kill signal to solver *)

  val to_array : sexp -> (sexp * sexp) list * sexp
end = struct
  type t = {
    command : sexp -> sexp;
    stop : unit -> unit;
  ...
  let stop s = s.stop ()
  let force_stop s = s.force_stop ()

  (** Issue a command and wait for it to succeed. Throws
      {! UnexpectedSolverResponse} *)
  ...
  (** Check if the current set of assumptions are consistent. Throws
      {!UnexpectedSolverResponse}. *)
  let check s =
    match s.command (Sexp.parse_string "(check-sat)" |> Result.get_exn) with
    | `Atom "unsat" -> Unsat
    | `Atom "sat" -> Sat
  ...
  let get_model s =
    ...
    | Z3 -> (
        ...
        let add_binder bound x =
          match x with
          | `List [ `Atom v; _ty_or_term ] -> StringSet.add v bound
        ...
        let rec free bound vars x =
          match x with
          | `Atom a -> if StringSet.mem a bound then vars else a :: vars
        ...
        let check_def x =
          ...
          | `List [ _def_fun; `Atom name; `List args; _ret; def ] ->
              let bound = add_bound StringSet.empty args in
        ...
        | `List xs ->
            ...
            let add_dep mp (x, xs, e) = StringMap.add x (xs, e) mp in
            let deps = List.fold_left add_dep StringMap.empty defs in

            let processing = ref StringSet.empty in
            let processed = ref StringSet.empty in
            let decls = ref [] in
                  ...
                  if StringSet.mem x !processed then arrange xs
                    ...
                    begin if StringSet.mem x !processing then
                      ...
                      | Some (ds, e) ->
                          ...
                          arrange ds;
                          processing := StringSet.remove x !processing;
  ...
  (** Evalute the given expression in the current context. Only valid after a
      [Sat] result. Throws {!UnexpectedSolverResponse}. *)
  ...
  (** Try to decode an s-expression as an array. The result is `(is,v)` where is
      are (key,value) pairs, and `v` is the default array value. Throws
      {!UnexpectedSolverResponse}. *)
  let to_array (exp0 : sexp) : (sexp * sexp) list * sexp =
    let bad () = raise (UnexpectedSolverResponse exp0) in
    let rec loop is exp =
      match exp with
      | `List [ `List [ `Atom "as"; `Atom "const"; _ ]; k ] -> (List.rev is, k)
      | `List [ `Atom "store"; a; i; v ] -> loop ((i, v) :: is) a
      | _ -> bad ()
    in
    loop [] exp0
  ...
  let new_solver (cfg : solver_config) : solver =
    ...
    let send_command c =
      ...
      let ans =
        match Sexp.parse_chan in_buf with
        | Ok x -> x
        | Error _ -> `Atom (In_channel.input_all in_err_chan)
      in
      cfg.log.receive (Sexp.to_string ans);
      ans
    in
  ...
  (** A connection to a solver, specialized for evaluting in the context of a
      model *)
...
*)
  ...
  let add_assert ?name solver cmd =
    let cmd =
      match name with None -> assume cmd | Some n -> named n (assume cmd)
    in
    solver.command cmd

  (** Evaluate expr after a SAT result *)
  let eval_expr solver e = get_expr solver e

  (** Evaluate expr list after a SAT result *)
  let eval_exprs solver es = get_exprs solver es

  (** check sat *)
  let check solver = check solver

  (** [set_option opt val] sets option [opt] to value [val]. *)
  let set_option solver x y = add_command solver (set_option x y)

  (** Set the logic to use. *)
  let set_logic solver l = add_command solver (set_logic l)

  (** Push a new scope. *)
  let push ?(n = 1) solver = add_command solver @@ push n

  (** Pop a scope. *)
  let pop ?(n = 1) solver = add_command solver @@ pop n
end
