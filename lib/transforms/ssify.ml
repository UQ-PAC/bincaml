  ...
  let split variable i_up i_down proc = 
    let cfg = match Procedure.graph proc with 
      ...
      let s_up = [] in
          (* Checks whether a program point is a join node by checking the number of predecessors *)
          let is_join block =
              if Procedure.get_blocks_pred proc block |> List.length > 1 then true else false in
                  let split_up i = if is_join i then s_up :: ( 
...
=======
          let contains_def vert variable = 
              ...
              Procedure.Vert.Begin succ_id -> succ_id |>
                Procedure.find_block proc
                |> Block.assigned_vars_iter 
                |> Iter.to_list 
                |> List.mem variable
              | Procedure.Vert.End pred_id -> pred_id |>
                Procedure.find_block proc
                |> Block.assigned_vars_iter 
                |> Iter.to_list 
                |> List.mem variable
