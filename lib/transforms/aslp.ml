...
module Bincaml_IBI = struct
  ...
  let extract_bits : bitvector -> bigint -> bigint -> bitvector = fun _ -> failwith ""

  let f_Elem_set :
    bigint -> bigint -> bitvector -> bigint -> bigint -> bitvector -> bitvector = fun _ -> failwith ""
  (** [f_Elem_set operand_width elem_width operand elem_index elem_width elem]
  *)

  let f_eq_bits : bigint -> bitvector -> bitvector -> bool = fun _ -> failwith ""
  let f_ne_bits : bigint -> bitvector -> bitvector -> bool = fun _ -> failwith ""
  let f_add_bits : bigint -> bitvector -> bitvector -> bitvector = fun _ -> failwith ""
  let f_sub_bits : bigint -> bitvector -> bitvector -> bitvector = fun _ -> failwith ""
  let f_mul_bits : bigint -> bitvector -> bitvector -> bitvector = fun _ -> failwith ""
  let f_and_bits : bigint -> bitvector -> bitvector -> bitvector = fun _ -> failwith ""
  let f_or_bits : bigint -> bitvector -> bitvector -> bitvector = fun _ -> failwith ""
  let f_eor_bits : bigint -> bitvector -> bitvector -> bitvector = fun _ -> failwith ""
  let f_not_bits : bigint -> bitvector -> bitvector = fun _ -> failwith ""
  let f_slt_bits : bigint -> bitvector -> bitvector -> bool = fun _ -> failwith ""
  let f_sle_bits : bigint -> bitvector -> bitvector -> bool = fun _ -> failwith ""
  let f_zeros_bits : bigint -> bitvector = fun _ -> failwith ""
  let f_ones_bits : bigint -> bitvector = fun _ -> failwith ""

  let f_replicate_bits : bigint -> bigint -> bitvector -> bigint -> bitvector = fun _ -> failwith ""
  (** [f_replicate_bits operand_width num_replications operand num_replications]
  *)

  let f_append_bits : bigint -> bigint -> bitvector -> bitvector -> bitvector = fun _ -> failwith ""
  (** [f_append_bits w1 w2 x1 x2] *)

  let f_ZeroExtend : bigint -> bigint -> bitvector -> bigint -> bitvector = fun _ -> failwith ""
  (** [f_ZeroExtend operand_width result_width operand result_width] *)

  let f_SignExtend : bigint -> bigint -> bitvector -> bigint -> bitvector = fun _ -> failwith ""
  (** [f_SignExtend operand_width result_width operand result_width] *)

  let f_lsl_bits : bigint -> bigint -> bitvector -> bitvector -> bitvector = fun _ -> failwith ""
  (** [f_lsl_bits operand_width shift_width operand shift] *)

  let f_lsr_bits : bigint -> bigint -> bitvector -> bitvector -> bitvector = fun _ -> failwith ""
  (** [f_lsr_bits operand_width shift_width operand shift] *)

  let f_asr_bits : bigint -> bigint -> bitvector -> bitvector -> bitvector = fun _ -> failwith ""
  ...
  let f_gen_Elem_read : bigint -> bigint -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_Elem_set : bigint -> bigint -> expr -> expr -> expr -> expr -> expr = fun _ -> failwith ""

  let f_gen_Mem_set : bigint -> expr -> expr -> expr -> expr -> unit = fun _ -> failwith ""
  (** [f_gen_Mem_set size address size acctype value] *)

  let f_gen_Mem_read : bigint -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  ...
  let f_gen_AArch64_MemTag_set : expr -> expr -> expr -> unit = fun _ -> failwith ""
  ...
  let f_gen_append_bits : bigint -> bigint -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_lsr_bits : bigint -> bigint -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_lsl_bits : bigint -> bigint -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_asr_bits : bigint -> bigint -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_replicate_bits : bigint -> bigint -> expr -> expr -> expr = fun _ -> failwith ""
  (** [f_gen_replicate_bits operand_width num_replications operand num_replications] *)

  let f_gen_ZeroExtend : bigint -> bigint -> expr -> expr -> expr = fun _ -> failwith ""
  (** [f_gen_ZeroExtend operand_width result_width operand result_width] *)

  let f_gen_SignExtend : bigint -> bigint -> expr -> expr -> expr = fun _ -> failwith ""
  (** [f_gen_SignExtend operand_width result_width operand result_width] *)

  let f_gen_slice : expr -> bigint -> bigint -> expr = fun _ -> failwith ""
  let f_gen_FPCompare : bigint -> expr -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPCompareEQ : bigint -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPCompareGE : bigint -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPCompareGT : bigint -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPAdd : bigint -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPSub : bigint -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPMulAdd : bigint -> expr -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPMulAddH : bigint -> expr -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPMulX : bigint -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPMul : bigint -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPDiv : bigint -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPMin : bigint -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPMinNum : bigint -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPMax : bigint -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPMaxNum : bigint -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPRecpX : bigint -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPSqrt : bigint -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPRecipEstimate : bigint -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_UnsignedRSqrtEstimate : bigint -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPRSqrtEstimate : bigint -> expr -> expr -> expr = fun _ -> failwith ""
  ...
  let f_gen_FPRecipStepFused : bigint -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPRSqrtStepFused : bigint -> expr -> expr -> expr = fun _ -> failwith ""

  let f_gen_FPToFixed :
    bigint -> bigint -> expr -> expr -> expr -> expr -> expr -> expr = fun _ -> failwith ""

  let f_gen_FixedToFP :
    bigint -> bigint -> expr -> expr -> expr -> expr -> expr -> expr = fun _ -> failwith ""

  let f_gen_FPConvert : bigint -> bigint -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPRoundInt : bigint -> expr -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPRoundIntN : bigint -> expr -> expr -> expr -> expr -> expr = fun _ -> failwith ""
  let f_gen_FPToFixedJS_impl : bigint -> bigint -> expr -> expr -> expr -> expr = fun _ -> failwith ""
end
