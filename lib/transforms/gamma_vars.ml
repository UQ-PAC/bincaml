...
let transform_proc ?(check_names = false) (add : ID.t -> Var.t -> bool) prog
  ...
  (* keep track of introduced vars *)
  let added = ref VarSet.empty in

  let gamma_of v =
    let args, r = Types.uncurry (Var.typ v) in
    let name = "Gamma_" ^ Var.name v in
    ...
    Block.map
      ...
      (function
        ...
        | Instr_IntrinCall { lhs; name; args; attrib } ->
            (* cursed *)
            let to_sm lhs =
              List.mapi (fun i v -> (Int.to_string i, v)) lhs
              |> StringMap.of_list
            in
            let of_sm lhs k =
...
let transform ?(check_names = false) (p : Program.t) =
  ...
  |> Iter.fold
       (fun prog (pid, proc) ->
         let prog, proc =
           transform_proc ~check_names (fun pid v -> true) p proc
         in
         let prog = Program.update_proc pid (fun _ -> Some proc) prog in
         prog)
       p
