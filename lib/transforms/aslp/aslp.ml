...
let insert_one_diamond proc dia : Program.proc =
  dia
...
let transform_block proc (b : Program.bloc) =
  ...
  in
  let diamonds =
    lift_code_block (module I) ~address Iter.(of_list opcodes |> map fst)
  in

  let proc = insert_one_diamond proc (List.hd diamonds) in
